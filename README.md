...
| 模块 | 能力 |
|---|---|
| 岗位导入 | 手动填写、粘贴 JD、TXT/PDF/DOCX、公开 URL 浏览器辅助预览、可选 Chrome Clipper；保存前结构化和去重。 |
...
- 不上传或导出 Cookie、登录态和其他求职者资料，也不自动投递；浏览器辅助导入的专用会话仅保存在本地 Git 忽略目录。
- 不做多用户账号、支付或企业招聘后台；这是单用户、本地优先工具。

粘贴公开岗位 URL 时，默认使用本机专用 Chrome 渲染页面；首次遇到登录或安全验证时由用户在可见窗口中完成，系统不绕过验证。
...
- Chrome：用于公开 URL 的浏览器辅助解析；Clipper 插件仍为可选入口
...
INTERNPILOT_BROWSER_PROFILE_DIR=data/browser_profile
INTERNPILOT_BROWSER_TIMEOUT=150
```
...
3. 粘贴公开 URL，由专用 Chrome 打开并提取渲染后的岗位正文；
4. 可选使用 Clipper 剪藏当前正在浏览的招聘页。
...
│   ├── browser_job_importer.py # 可见 Chrome URL 解析与任务状态
...
- URL 浏览器导入校验初始地址及跳转后的地址，拒绝本机、内网和保留地址；一次只处理一个用户主动提交的链接。
