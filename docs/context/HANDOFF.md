...
- 前端：AI 助手显示本轮检索来源。
- URL 导入：前端粘贴链接默认使用 Playwright 专用可见 Chrome；任务可轮询和取消，浏览器会话保存在 Git 忽略目录，登录/验证码由用户手工完成。旧 HTTP 抓取接口仅作兼容。
...
- 验证：`compileall`、`git diff --check`、安全检查通过；浏览器导入提交范围 pytest 41 项通过（12 条第三方依赖弃用警告）。专用 Chrome 启动已验证；BOSS 正文验收仍受系统残留 Fake-IP `198.18.0.51` 阻塞。

## 当前优先级

1. 恢复 Clash 代理或刷新系统 DNS 后，重启 InternPilot 并用真实岗位链接验收浏览器辅助预览。
2. 在本机浏览器手工验收匿名 Demo 与详情页 taxonomy 合并界面。
3. 由项目所有者在 Render 账户中授权仓库并创建 Demo 服务；按 `docs/DEPLOYMENT.md` 验收。
