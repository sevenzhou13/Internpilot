...
- 真值标签：`job_category_labels` 表内的人工复核类别，或已完成八类口径映射的 `external_dataset` 外部标签；训练记录必须分别统计两种来源。
- 弱标签：规则结构化得到的 `jobs.job_category` 或 `llm_mapped` 的 LLM 推断类别；只可用于探索性训练，报告必须单独说明，且不输出可报告的独立评估指标。
...
- 代码和最小样本端到端测试已通过；公开候选 JD 不带岗位类别时仍需复核，带外部类别字段的数据可在完成映射与抽查后训练。
...
- 外部类别口径：类别映射错误会污染训练集；保存映射表和数据版本，并对每个映射类抽查至少 10% 且不少于 5 条。
